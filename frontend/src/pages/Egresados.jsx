        ...
        <div className="flex flex-col md:flex-row gap-4 items-end">
          <div className="w-full md:w-64">
            <Select
              label="Programa"
              value={programaId}
              onChange={(e) => { setProgramaId(e.target.value); setBloqueId(''); setCohorteId(''); setData(null); }}
          ...
          <div className="w-full md:w-64">
            <Select
              label="Bloque"
              value={bloqueId}
              onChange={(e) => { setBloqueId(e.target.value); setData(null); }}
          ...
          <div className="border-b border-indigo-500/20 pb-4 mb-4 flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-white">
                {data.programa?.nombre || 'Todos los programas'} {data.cohorte_id ? `(Cohorte ${data.cohorte_id})` : ''}
              </h3>
              <p className="text-sm text-indigo-300">
                Bloques requeridos: {data.overall?.total_bloques_requeridos} —
                <span className="text-green-400 font-bold mx-1">
                  Egresados: {data.overall?.graduados} de {data.overall?.total_estudiantes} ({Math.round((data.overall?.rate || 0) * 100)}%)
                </span>
              </p>
            </div>
          </div>
